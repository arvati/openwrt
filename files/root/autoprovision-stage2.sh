...
EOF
	...
	mkdir /overlay-boot
	uci get fstab.overlay-boot && uci delete fstab.overlay-boot
	uci set fstab.overlay-boot=mount
	uci set fstab.overlay-boot.target='/overlay-boot'
	uci set fstab.overlay-boot.fstype='jffs2'
	uci set fstab.overlay-boot.device='/dev/mtdblock6'
	uci set fstab.overlay-boot.options='rw,sync'
	uci set fstab.overlay-boot.enabled='1'
	uci commit fstab
